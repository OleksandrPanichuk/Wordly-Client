...
const getSubscription = async () => {
	return await fetcher.getOrNull<TypeSubscription>(ApiRoutes.BILLING.SUBSCRIPTION)
}

const getPayments = async () => {
	return await fetcher.getOrDefault<TypePayment[]>(ApiRoutes.BILLING.PAYMENTS, [])
}

const getPayment = async (paymentId: string) => {
	return await fetcher.getOrNull<TypePayment>(ApiRoutes.BILLING.PAYMENT(paymentId))
}
